...
public class CsrfFilter implements Filter {
    ...
    private List<String> domainPath;

    public void setDomainPath(List<String> domainPath) {
    ...
    public void doFilter(ServletRequest servletRequest, ServletResponse servletResponse, FilterChain filterChain)
        ...
        boolean isCsrf = true;
        for (String serverName : domainPath) {
            serverNameTemp = serverNameTemp + "," + serverName;
            if ((null != referer && referer.startsWith(serverName))) {
                isCsrf = false;
            }
        }
        if (isCsrf) {
            log.error("CSRF过滤器 => 服务器：{} => 当前域名：{}", serverNameTemp.substring(1), referer);
